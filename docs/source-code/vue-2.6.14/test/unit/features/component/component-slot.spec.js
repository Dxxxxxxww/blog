...
describe('Component slot', () => {
  ...
  it('should handle nested components in slots properly', done => {
    ...
    expect(vm.$el.innerHTML).toBe(
      `**<div><div>foo</div></div> <div>bar<div>foo</div></div>**`
    )
  ...
  it('should update named slot correctly when its position in the tree changed', done => {
    ...
    expect(vm.$el.firstChild.innerHTML).toBe('<span><span>**1**</span></span>')
    parentVm.alter = false
    waitForUpdate(() => {
      vm.message = 2
    })
      .then(() => {
        expect(vm.$el.firstChild.innerHTML).toBe('<span>**2**</span>')
      })
      .then(done)
  })
