...
有数组 [1,2,3,1,'1',2,'2',3] 要求去重后返回 [1,2,3,'1','2']

1. Set 去重
2. Map 去重，这里只能用 map 不能用对象 key ，因为 对象 key 不能区分数字和字符串
3. filter + indexOf 判断 index 下标是否相同  arr.filter((item, index) => index === arr.indexOf(item))
4. reduce + includes arr.reduce((res, item) => { if (!res.includes(item) { res.push(item) }  return res}, [])
